...
const Sidebar: React.FC<SidebarProps> = ({ collapsed, onToggle }) => {
  ...
  const linkClass = ({ isActive }: { isActive: boolean }) =>
      `flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all ${
          isActive
              ? 'bg-[#5b4fff]/15 text-[#968fff] shadow-sm border border-[#5b4fff]/20'
              : 'text-zinc-400 hover:text-white hover:bg-zinc-800/50'
      }`;

  return (
      <aside
          className={`flex flex-col bg-[#111111]/85 backdrop-blur-xl border-r border-zinc-800/60 transition-all duration-300 ${
              collapsed ? 'w-17' : 'w-64'
          }`}
      >
        {/* Logo */}
        <div className="flex items-center gap-3 px-4 py-5 border-b border-zinc-800/60">
          <div className="shrink-0 flex items-center justify-center w-9 h-9 rounded-xl bg-linear-to-br from-[#5b4fff] to-[#968fff] shadow-md shadow-[#5b4fff]/20">
